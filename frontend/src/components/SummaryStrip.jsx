...
import {
  ...
  fetchPods, fetchStrategies, updateStrategy,
...
function InternalTransfersTab({ queryClient, capitalData }) {
  ...
  // Strategies — used to find the Darwinex strategy linked to a brokerage
  // account (from_account on an outbound leg), for the "Final withdrawal"
  // auto-close + P&L-preview feature below. See README §14.
  ...
  const strategyByAccount = Object.fromEntries(
    allStrategies.filter(s => s.brokerage_account).map(s => [s.brokerage_account, s])
  )
  ...
  function openAdd() {
    ...
    setFormCapRet(''); setFormProfLo(''); setFormIsFinal(false); setFormError(null); setShowForm(true)
  }
  ...
  // Final-withdrawal P&L preview: baseline = all-time Wallet->account
  // deployed (never reduced); returned = all-time account->Wallet legs,
  // EXCLUDING the record being edited (its new amount is added back in),
  // PLUS the amount currently being entered. pnl = returned - baseline —
  // identical formula to the backend's _darwinex_closed_strategy_agg, so
  // the preview always matches what the dashboard will show once saved.
  // See README §14.
  function finalWithdrawalPreview(account, amt, excludeId) {
    const baseline = transfers
      .filter(t => t.to_account === account && t.from_account === 'Wallet')
      .reduce((s, t) => s + t.amount, 0)
    const returnedSoFar = transfers
      .filter(t => t.from_account === account && t.to_account === 'Wallet' && t.id !== excludeId)
      .reduce((s, t) => s + t.amount, 0)
    const returned = returnedSoFar + (amt || 0)
    return { baseline: round2(baseline), returned: round2(returned), pnl: round2(returned - baseline) }
  }
  ...
  async function handleAdd(e) {
    ...
    if (isNaN(amt) || amt <= 0) return setFormError('Enter valid amount > 0')
    if (!formDate) return setFormError('Select a date')
    // Final withdrawal: locked as pure Capital Return — real P&L comes from
    // the backend's all-time returned-vs-deployed calc, never a per-leg
    // guess (see finalWithdrawalPreview above / README §14).
    const split = formIsFinal
      ? { ok: true, cr: parseFloat(amt.toFixed(2)), pl: 0 }
      : validateSplit(amt, formCapRet, formProfLo)
    ...
    try {
      ...
      if (formIsFinal && strategyByAccount[formFrom]) {
        await updateStrategy(strategyByAccount[formFrom].id, { status: 'Closed' })
        await queryClient.invalidateQueries({ queryKey: ['strategies_all'] })
      }
  ...
  async function handleUpdate(e) {
    ...
    if (isNaN(amt) || amt <= 0) return setEditError('Enter valid amount > 0')
    if (!editDate) return setEditError('Select a date')
    const split = editIsFinal
      ? { ok: true, cr: parseFloat(amt.toFixed(2)), pl: 0 }
      : validateSplit(amt, editCapRet, editProfLo)
    ...
    try {
      await updateInternalTransfer(editRec.id, { transfer_date: editDate, from_account: editFrom,
        ...
        capital_return_amount: split.cr, profit_loss_amount: split.pl })
      if (editIsFinal && strategyByAccount[editFrom]) {
        await updateStrategy(strategyByAccount[editFrom].id, { status: 'Closed' })
        await queryClient.invalidateQueries({ queryKey: ['strategies_all'] })
      }
      await queryClient.invalidateQueries({ queryKey: ['internal_transfers'] })
      await queryClient.invalidateQueries({ queryKey: ['portfolio'] })
  ...
  function renderTransferForm(isEdit) {
    ...
    const isFinal = isEdit ? editIsFinal : formIsFinal
    const error   = isEdit ? editError  : formError
    const saving  = isEdit ? editSaving : submitting
    const setDate = isEdit ? setEditDate   : setFormDate
    ...
    const setIsFinal = isEdit ? setEditIsFinal : setFormIsFinal
    const onSubmit = isEdit ? handleUpdate : handleAdd
    ...
    const linkedStrategy = strategyByAccount[from]
    const preview = (classifiable && isFinal)
      ? finalWithdrawalPreview(from, amtNum, isEdit ? editRec?.id : undefined)
      : null
          ...
          <div style={{
            ...
            <label style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer' }}>
              ...
              <span style={{ fontSize: 11.5, fontWeight: 600, color: '#E2E8F0' }}>
                Final withdrawal — closes {linkedStrategy.name}
              </span>
            </label>
            <p style={{ fontSize: 10, color: '#64748B', marginTop: 6, marginLeft: 23 }}>
              Locks this leg as pure Capital Return. Sets {linkedStrategy.name} to Closed on save.
              Real P&amp;L is computed automatically from all-time deployed vs returned.
            </p>
            {preview && (
              <div style={{ marginTop: 8, marginLeft: 23, fontSize: 11.5 }}>
                Deployed <b style={{ color: '#38BDF8' }}>{formatCurrencyAbs(preview.baseline)}</b> · Returned{' '}
                <b style={{ color: '#38BDF8' }}>{formatCurrencyAbs(preview.returned)}</b> · Resulting P&amp;L{' '}
                <b style={{ color: preview.pnl >= 0 ? '#34D399' : '#F87171' }}>
                  {preview.pnl >= 0 ? '+' : ''}{formatCurrencyAbs(preview.pnl)} {preview.pnl >= 0 ? '(profit)' : '(loss)'}
                </b>
              </div>
            )}
          </div>
          ...
          <div style={{
            ...
            {linkedStrategy && (
              <p style={{ fontSize: 10, color: '#64748B', marginBottom: 8 }}>
                Only for a partial/early profit take while the strategy is still running. Use
                "Final withdrawal" above instead once all money is coming out.
              </p>
            )}
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: 10 }}>
              ...
              <div>
                ...
                <AmountInput value={profLo} onChange={setProfLo} style={INPUT_STYLE} placeholder="e.g. -23776.74" />
              </div>
